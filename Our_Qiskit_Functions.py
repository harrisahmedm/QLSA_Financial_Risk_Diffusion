...
def Wavefunction(obj, **kwargs):
    ...
    #For Debugging
    #print("\nqubits = " + str(qubits))
    #print("\nstatevec = " + str(statevec))
    
    for i in range(int(len(statevec))):
        value = round(statevec[i].real, dec) + round(statevec[i].imag, dec) * 1j
        if (value.real != 0) or (value.imag != 0):
            state = list(Binary(int(i), int(2**qubits), 'R')) #Changed 'L' to 'R' for Qiskit-friendly notation
            state_str = ''
            ...
            if sys == True:
                # Reverse systems ordering for Qiskit-style display
                systems_rev = systems[::-1]
                show_systems_rev = show_systems[::-1]
                k = 0
                for s in np.arange(len(systems_rev)):

                    if show_systems_rev[s] == True:

                        if int(s) != last_sys:
                            state.insert(int(k + systems_rev[s]), '>|')
                            k = int(k + systems_rev[s] + 1)
                        else:
                            k = int(k + systems_rev[s])
                    else:
                        for s2 in np.arange(systems_rev[s]):
                            del state[int(k)]
            ...
            #Appending instead of prepending to adhere with Qiskit ordering
            for j in np.arange(len(state)):
                if type(state[j]) != str:
